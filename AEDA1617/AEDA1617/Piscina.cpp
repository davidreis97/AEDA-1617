...
bool Piscina::removeUtente(int id) {
	for (int j = 0; j < this->horario.size(); j++) {
		for (int p = 0; p < this->horario[j].getPeriodos()->size(); p++) {
			for (vector<Utente>::iterator itr = this->horario[j].getPeriodos()->at(p).getUtentes()->begin(); itr != this->horario[j].getPeriodos()->at(p).getUtentes()->end();) {
				if ((*itr).getId() == id) {
					this->horario[j].getPeriodos()->at(p).getUtentes()->erase(itr);
				}
				else {
					itr++;
				}
		...
		for (int p = 0; p < this->horario[j].getAulas()->size(); p++) {
			for (vector<Utente>::iterator itr = this->horario[j].getAulas()->at(p).getUtentes()->begin(); itr != this->horario[j].getAulas()->at(p).getUtentes()->end();) {
				if ((*itr).getId() == id) {
					this->horario[j].getAulas()->at(p).getUtentes()->erase(itr);
				}
				else {
					itr++;
				}
